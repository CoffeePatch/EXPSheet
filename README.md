...
| `Time` | Time of the expense (`HH:mm`). Also accepts shorthand like `530p`, `5:30pm`, or `5p` and normalises to `HH:mm`. Leave blank to fall back to Timestamp time. |
...
| `Split Person` | Comma-, semicolon-, or dot-separated list of people sharing the expense. Use `me` for yourself. Defaults to `me` if blank. |
...
| `Time` | Time/Text | `HH:mm` preferred; also supports `530p`, `5:30pm`, `5p` and normalises to `HH:mm` | Optional | Transaction + Transfer |
...
| `Split Person` | Text list | One or many people separated by comma, semicolon, dot, or new line | Optional (defaults to `me`) | Transaction |
...
### Split logic
- The `Split Person` field accepts a comma-, semicolon-, or dot-separated list (e.g., `me, Alice, Bob` or `me.Alice.Bob`).
